...
export class CRUDProjService {
  ...
  // دالة جديدة لجلب التبرعات
  GetPaginatedMonetaryDonations(pageNumber: number, pageSize: number, orderByDirection: number = 1): Observable<any> {
    return this._HttpClient.get<any>(`${Environment.baseUrl}${Environment.VersionUrl}Payment/GetPaginatedMonetaryDonations?pageSize=${pageSize}&pageNumber=${pageNumber}&orderByDirection=${orderByDirection}`);
  }
}